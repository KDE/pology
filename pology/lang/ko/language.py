...
PLURAL_JOSA_RE = ur'(도|로|만|에|은|을|이|의)'
# 많이 등장하는 명사만 쓴다
PLURAL_NOUN_RE = ur'(값|개발자|것|기능|기술|그림|글자|답변|디렉터리|메시지|' \
                 ur'문서|문자|분|사람|사진|설정|언어|점|질문|파일|팀|패키지|' \
                 ur'폴더|프로그램|플러그인|항목|활동)'
PLURAL_EXCEPT = ur'(핸들|번들)' + PLURAL_JOSA_RE

PLURAL_R = re.compile(ur'(' +
        ur'[\uac00-\ud7a3]+\(들\)' + ur'|' +
        PLURAL_NOUN_RE + ur'들' + ur'|' +
        ur'[\uac00-\ud7a3]+들' + PLURAL_JOSA_RE + ur'\s' + ur')', re.UNICODE)
...
SYLLABLE_WITH_T_RIEUL = '[%s]' % ''.join([unicode(c) for c in
                                          range(0xAC00 + 8, 0xD7A4, 28)])
SYLLABLE_WITH_T_SSANGSIOS = '[%s]' % \
                            ''.join([unicode(c) for c in
...
def hal_geosipnida (msgstr, msg, cat):
    ...
    return check(GEOSIPNIDA_R, GEOSIPNIDA_EXCLUDE, msgstr, msg, cat, "Try to revise '%(word)s'.")
